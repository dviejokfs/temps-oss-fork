...
impl ProviderPreferenceService {
    ...
    /// - `interactive_bridge_enabled = Some(true)` is only valid when
    ///   `provider_type == "agent_cli"` AND `agent_cli_provider_id == "claude_cli"`.
    ///   For all other provider configurations the flag is auto-reset to `false`.
    /// - `interactive_bridge_enabled = None` preserves the existing value in the DB
    ///   on update; on insert it defaults to `false`.
    ///
    /// If a row already exists for `scope`, only `provider_type`,
    /// `agent_cli_provider_id`, and (when not `None`) `interactive_bridge_enabled`
    /// are mutated; governance limit columns are left untouched.
    ...
    pub async fn set(
        ...
        interactive_bridge_enabled: Option<bool>,
        ...
        // The interactive bridge is Claude CLI-specific.  Reject an explicit
        // enable request when the resolved provider is not claude_cli, and
        // auto-disable it when switching away from claude_cli.
        let is_claude_cli =
            provider_type == "agent_cli" && agent_cli_provider_id.as_deref() == Some("claude_cli");
        if !is_claude_cli && interactive_bridge_enabled == Some(true) {
            return Err(ProviderPreferenceError::Validation {
                message:
                    "interactive_bridge_enabled can only be set to true when the active provider \
                     is 'claude_cli' (Codex and OpenCode have no interactive control protocol)"
                        .to_string(),
            });
        }
        // When switching away from claude_cli, force the flag off regardless of
        // what the caller sent — so the DB never stores an "enabled" state that
        // has no effect.  When staying on claude_cli, use the caller's value (or
        // None = preserve existing).
        let resolved_bridge_enabled: Option<bool> = if !is_claude_cli {
            Some(false) // force-disable
        } else {
            interactive_bridge_enabled // None = preserve, Some(v) = set
        };
        ...
        if let Some(existing) = self.get(scope).await? {
            ...
            if let Some(bridge_enabled) = resolved_bridge_enabled {
                active.interactive_bridge_enabled = Set(bridge_enabled);
            }
            active
            ...
            let model = ai_gateway_config::ActiveModel {
                ...
                interactive_bridge_enabled: Set(resolved_bridge_enabled.unwrap_or(false)),
                ..Default::default()
            };
            model
        ...
        for (id, _label) in temps_agents::ai_cli::PROVIDER_NAMES {
            if let Some(provider) = temps_agents::ai_cli::create_provider(id) {
                ...
                if status.installed && status.authenticated {
                    host_authenticated.push((*id).to_string());
                }
            }
        }
...
mod tests {
    ...
    // -----------------------------------------------------------------------
    // interactive_bridge_enabled validation (ADR-038 Phase 2, deliverable 2)
    // -----------------------------------------------------------------------

    /// Trying to opt in to the interactive bridge when the provider is NOT
    /// `claude_cli` must be rejected with a `Validation` error.  The flag is
    /// protocol-specific — no other agent CLI has a `--permission-prompt-tool
    /// stdio` implementation.
    #[tokio::test]
    async fn test_set_interactive_bridge_enabled_true_without_claude_cli_returns_error() {
        // Validation fires before any DB access; no query results needed.
        let db = MockDatabase::new(DatabaseBackend::Postgres).into_connection();
        let db = Arc::new(db);
        let service = ProviderPreferenceService::new(db.clone(), test_provider_key_service(db));

        // Case 1: gateway provider
        let result = service
            .set("instance", "gateway".to_string(), None, Some(true))
            .await;
        let err = result.unwrap_err();
        assert!(
            matches!(err, ProviderPreferenceError::Validation { .. }),
            "expected Validation, got: {err:?}"
        );
        let msg = err.to_string();
        assert!(
            msg.contains("interactive_bridge_enabled"),
            "error message should reference the flag: {msg}"
        );
        assert!(
            msg.contains("claude_cli"),
            "error message should mention claude_cli: {msg}"
        );
    }

    /// Opting in to the interactive bridge with `claude_cli` as the provider
    /// must pass validation and proceed to the DB write.
    #[tokio::test]
    async fn test_set_interactive_bridge_enabled_true_with_claude_cli_succeeds() {
        let inserted = ai_gateway_config::Model {
            id: 1,
            scope: "instance".to_string(),
            allowed_models: None,
        ...
        let db = MockDatabase::new(DatabaseBackend::Postgres)
            .append_query_results(vec![
                Vec::<ai_gateway_config::Model>::new(), // SELECT → no existing row
                vec![inserted],                         // INSERT → returned model
            ])
            .into_connection();
        ...
        assert!(
            ...
            "model.interactive_bridge_enabled should be true"
        );
    }

    /// When the operator switches FROM `claude_cli` TO any other provider
    /// (`gateway` or a different agent CLI), `interactive_bridge_enabled` must
    /// be force-set to `false` regardless of what the caller passes — the
    /// bridge is meaningless without claude_cli and must not silently stay on.
    #[tokio::test]
    async fn test_set_auto_disables_bridge_when_switching_away_from_claude_cli() {
        ...
        // Switch to gateway — the caller does NOT explicitly request disabling,
        // but the service must auto-disable it.
        let result = service
            .set("instance", "gateway".to_string(), None, None)
            .await;
