...
export function registerProjectsCommands(program: Command): void {
  ...
  projects
    ...
    .option('--public-readiness-timeout <seconds>', 'Seconds to wait for the public URL to respond before reverting a deployment (10-900). Raise this for apps that do real work after the container starts (e.g. compiling/bundling source on boot) before they are reachable')
    .option('--enable-public-readiness-check', 'Require the public URL to respond before a deployment is marked complete (platform default)')
    .option('--disable-public-readiness-check', 'Skip the post-deploy public URL check entirely — for apps that do not serve plain HTTP even when healthy')
    .option('--json', 'Output in JSON format')
    .option('-y, --yes', 'Skip prompts (for automation)')
