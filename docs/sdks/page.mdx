...
`ingestKey` is sent as the `X-Temps-Analytics-Key` header on every request. This alone works — but every request now crosses origins, which brings in two constraints worth designing around: CORS (Temps' ingest endpoints answer with `Access-Control-Allow-Origin: *` and no credentials, so cookies never round-trip) and `navigator.sendBeacon`, used for the `page_leave` event on unload, which cannot set custom headers at all. Both are why step 3 is recommended over calling the backend directly.
...
async function handler(req: Request, { params }: { params: { path: string[] } }) {
  ...
  const target = `${TEMPS_BACKEND}/${params.path.join('/')}${url.search}`;
